...
def updateInState(window):
    inStr = 'INPUT: ' + \
        ...
        str(IN_STATE[7]) + ' ' + \
        str(IN_STATE[8])
    window.find_element('-AIO-INPUT-').update(inStr)
    window.find_element('-AIO-INPUT-2-').update(inStr)
...
def updateOutState(window):
    outStr = 'OUTPUT: ' + \
        ...
        str(OUT_STATE[7]) + ' ' + \
        str(OUT_STATE[8])
    window.find_element('-AIO-OUTPUT-').update(outStr)
    window.find_element('-AIO-OUTPUT-2-').update(outStr)
...
def waitInputState(inPort, state, window):
    ...
    while currentInState != state:                      # while state of input doesn't equal given state keep checking
        if not (program_state.LIVE_MODE or program_state.THRESH_MODE) and not program_state.RUN_MODE:                  # if user requests program stop break out of the check
            return False
